...
function getTools() {
  ...
  return [
    ...
    },
    {
      ...
      description:
        'Read the current orchestration status and result summary from session-local state, optionally waiting for completion.',
      inputSchema: {
        type: 'object',
        properties: {
          waitForCompletion: { type: 'boolean' },
...
function assertNoSensitiveLocalPathSegments(candidatePath, rootPath, label) {
  ...
  for (const segment of rootSegments) {
    ...
    if (SENSITIVE_LOCAL_PATH_SEGMENTS.has(normalizedSegment)) {
...
async function handleGetReplay(toolArgs = {}) {
  const session = requireLatestReplay();
  if (toolArgs.waitForCompletion === true && activeReplayTask) {
...
async function handleGetOrchestration(toolArgs = {}) {
  const session = requireLatestOrchestration();
  if (toolArgs.waitForCompletion === true && activeOrchestrationTask) {
    await activeOrchestrationTask;
  }
  return formatOrchestrationSummary(session);
}
